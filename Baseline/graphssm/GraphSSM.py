...
from .ssm import InterpolationTokenMixer, Conv1DTokenMixer
...
# model = DiagonalSSM(
#     data.x.size(-1),  # in_ch
#     data.y.max().item()+1,  # out_ch
#     hidden_channels=config["hidden_channels"],  # 32
#     ssm_format=config["ssm_format"],  # siso
#     token_mixer=config["token_mixer"],  # 'interp'
...
class DiagonalS6SSM(nn.Module):
    ...
    def __init__(
        ...
        in_channels = parms.enc_dims[0]
        out_channels = parms.enc_dims[1]
    ...
    def forward(self, sup_list, feat_list, noise_list, pred_flag=True):
        ...
        xs = feat_list #[data.x for data in snapshots]
        ...
        for i in range(self.num_layers):
            ...
            for j, data in enumerate(sup_list):
                # xs[j] = self.convs[i](xs[j], data.edge_index)
                outs = self.convs[i](xs[j], data.coalesce().indices())
